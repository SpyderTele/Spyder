...
redis:mset(
Spyder..":VERSION",GetUser.information.Source_version,
Spyder..":SUDO_ID:",GetUser.information.id,
Spyder..":DataCenter:",GetUser.information.DataCenter,
Spyder..":UserNameBot:",BOT_User,
Spyder..":ApiSource",GetUser.information.WebSite,
Spyder..":NameBot:","الزعيم",
"TH3Spyder_INSTALL","Yes"
)
redis:hset(Spyder..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
info = {} 
...
ApiSpyder = redis:get(Spyder..":ApiSource")
...
..'¦ Api_Src->: \27[1;34m'..ApiSpyder..'\027[0;32m\n'
...
if not redis:get(Spyder..":Check_Bot:"..W) then
Rgz,res=https.request(ApiSpyder..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(Spyder..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type ~= "pv" and redis:get(Spyder..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
...
if not msg.Special and not msg.Admin and redis:get(Spyder.."lock_Add"..msg.chat_id_) then
...
if msg.Special or msg.Admin then redis:srem(Spyder..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(Spyder..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(Spyder) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.iSpyder(msg,{Text:match(Spyder)})
if GetMsg then
